...
from model.generation_utils import DreamGenerationMixin as BaseDreamGenerationMixin
import types
...
class Dream(LM):
    ...
    def _create_model_and_tokenizer(self, pretrained, dtype, trust_remote_code):
        ...
        self.model._sample = types.MethodType(BlockDreamGenerationMixin._sample_cache, self.model)
    ...
    def _generate_batch(self, prompts: List[str]) -> Tuple[List[str], Optional[List[int]]]:
        ...
        print('=' * 20, end='\n\n')
        return responses, generation_ids.nfe_history

    def generate_until(self, requests: List[Instance], disable_tqdm: bool = False):
        res = []
        total_nfe = []
        if self.use_cache: # block generation w/ cache
            self.model.diffusion_generate = types.MethodType(BlockDreamGenerationMixin.diffusion_generate, self.model)
            self.model._sample = types.MethodType(BlockDreamGenerationMixin._sample_cache, self.model)
        elif self.alg == 'confidence_threshold': # generation w/o cache
            self.model.diffusion_generate = types.MethodType(BlockDreamGenerationMixin.diffusion_generate, self.model)
            self.model._sample = types.MethodType(BlockDreamGenerationMixin._sample_parallel, self.model)
        elif self.alg == "entropy": # native Dream
            self.model.diffusion_generate = types.MethodType(BaseDreamGenerationMixin.diffusion_generate, self.model)
            self.model._sample = types.MethodType(BaseDreamGenerationMixin._sample, self.model)
        else:
        ...
        for batch_idx in range(0, len(requests), self.batch_size):
            ...
            responses, nfe_history = self._generate_batch(contexts)
            if nfe_history is not None:
