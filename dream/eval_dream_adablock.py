...
from model.generation_utils_adablock import DreamGenerationMixin
import types
...
class Dream(LM):
    def __init__(
        ...
        delimiter_threshold: Optional[float] = None,
        ...
        **kwargs,
        ...
        self.num_tokens = 0
        self.num_tokens_excluding_eos = 0
    ...
    def _create_model_and_tokenizer(self, pretrained, dtype, trust_remote_code):
        ...
        self.model._sample = types.MethodType(DreamGenerationMixin._sample_adablock, self.model)
    ...
    def _generate_batch(self, prompts: List[str]) -> Tuple[List[str], Optional[List[int]], Optional[List[int]]]:
        ...
        self.num_tokens_excluding_eos += (generated_sequence != self.tokenizer.eos_token_id).sum().item()
        responses = [
        ...
        print('=' * 20, end='\n\n')
        return responses, generation_ids.nfe_history, generation_ids.block_history

    def generate_until(self, requests: List[Instance], disable_tqdm: bool = False):
        res = []
        total_nfe = []
        num_blocks = []
        assert self.delimiter_threshold is not None, "delimiter_threshold is required for AdaBlock generation"
        if self.use_cache: # AdaBlock generation w/ cache
            self.model.diffusion_generate = types.MethodType(DreamGenerationMixin.diffusion_generate, self.model)
            self.model._sample = types.MethodType(DreamGenerationMixin._sample_adablock_cache, self.model)
            self.model._compute_block_length = types.MethodType(DreamGenerationMixin._compute_block_length, self.model)
        elif self.alg == 'confidence_threshold': # AdaBlock generation w/o cache
            self.model.diffusion_generate = types.MethodType(DreamGenerationMixin.diffusion_generate, self.model)
            self.model._sample = types.MethodType(DreamGenerationMixin._sample_adablock, self.model)
            self.model._compute_block_length = types.MethodType(DreamGenerationMixin._compute_block_length, self.model)
        else:
        ...
        for batch_idx in range(0, len(requests), self.batch_size):
            ...
            responses, nfe_history, block_history = self._generate_batch(contexts)
