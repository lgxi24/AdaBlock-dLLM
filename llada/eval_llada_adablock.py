...
from generate_adablock import generate_adablock, generate_adablock_prefix_cache, generate_adablock_dual_cache
...
class LLaDAEvalHarness(LM):
    def __init__(
        ...
        **kwargs,
        ...
        if delimiter_ids is None:
            self.delimiter_ids = [198] if not extra_ids else extra_ids
        elif isinstance(delimiter_ids, str):
            delimiter_ids = delimiter_ids.strip('[]')
            if ',' in delimiter_ids:
                self.delimiter_ids = [int(x.strip()) for x in delimiter_ids.split(',')]
            else:
                self.delimiter_ids = [int(delimiter_ids)] + extra_ids
        else:
            self.delimiter_ids = [delimiter_ids] + extra_ids if extra_ids else [delimiter_ids]
    ...
    def generate_until(self, requests):
        ...
        for i, req in enumerate(tqdm(requests, desc="Generating...")):
            ...
            if self.use_cache:
                if self.dual_cache:
                    generated_answer, nfe_history, block_history = generate_adablock_dual_cache(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, init_block_length=self.block_length, 
                                    temperature=0, remasking=self.remasking, mask_id=self.mask_id, threshold=self.threshold, delimiter_ids=self.delimiter_ids, delimiter_threshold=self.delimiter_threshold)
                else:
                    generated_answer, nfe_history, block_history = generate_adablock_prefix_cache(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, init_block_length=self.block_length, 
                                    temperature=0, remasking=self.remasking, mask_id=self.mask_id, threshold=self.threshold, delimiter_ids=self.delimiter_ids, delimiter_threshold=self.delimiter_threshold)
            else:
                generated_answer, nfe_history, block_history = generate_adablock(self.model, input_ids, steps=self.steps, gen_length=self.gen_length, init_block_length=self.block_length, 
                                        temperature=0, remasking=self.remasking, mask_id=self.mask_id, threshold=self.threshold, delimiter_ids=self.delimiter_ids, delimiter_threshold=self.delimiter_threshold)
            ...
            if self.save_dir is not None:
                # save new generated answer to the file
            ...
            # if self.show_speed:
                ...
                # print(f"block_length for each block: {block_history}")
                # print('=' * 20, end='\n\n')
